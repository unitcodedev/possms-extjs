...
//        $query = $this->db->select('mk_pass, salt')
//                ->where('id', $id)
//                ->limit(1)
//                ->get($this->tables['ms_karyawan']);
        $query1 = "select * from ms_karyawan where id = '$id'";
        ...
        // bcrypt
        if ($use_sha1_override === FALSE && $this->hash_method == 'bcrypt') {
            if ($this->bcrypt->verify($password, $hash_password_db->mk_pass)) {
                return TRUE;
            }
        ...
        if ($db_password == $hash_password_db->mk_pass) {
            return TRUE;
        } else {
            return FALSE;
        }
     ...
     * @author Mathew
     * */
    public function activate($id, $status = 4) {
//		$this->trigger_events('pre_activate');
//
//		if ($status !== 4)
//		{
//			$query = $this->db->select($this->identity_column)
//			                  ->where('stat_karyawan', $status)
//			                  ->limit(1)
//			                  ->get($this->tables['ms_karyawan']);
//
//			$result = $query->row();
...
//
//			$this->trigger_events('extra_where');
////			$this->db->update($this->tables['ms_karyawan'], $data, array($this->identity_column => $identity));
//		}
//		else
//		{
//			$data = array(
...
//        $query = $this->db->select('sdm_id, mk_email, id, mk_pass, stat_karyawan, last_login')
//                ->where('sdm_id', $this->db->escape_str($identity))
//                ->from('ms_karyawan')
//                ->limit(1);
        $query1 = "select * from ms_karyawan where sdm_id = '$identity'";
        $query = $this->db->query($query1);
        ...
        if ($query->num_rows() === 1) {
            ...
            $password = $this->hash_password_db($user->sdm_id, $password);

            if ($password === TRUE) {
                if ($user->stat_karyawan == 5) {
                    $this->trigger_events('post_login_unsuccessful');
