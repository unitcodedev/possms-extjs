...
//        $max = $numrow 
    ...
    function get_bulan_server() {
        $sql = "SELECT to_char(localtimestamp,'mm') as bulan";
...
//        $sql = "select $field from $table order by $sort DESC limit 1";
    ...
    function cari_idrekanan($grouprekanan) {
        $sql = "select id_rekanan from ms_rekanan where id_grouprekanan = '$grouprekanan' and status = 2 order by CAST(coalesce(id_rekanan, '0') AS integer) DESC limit 1";
    ...
    function cari_grouprekanan($typerekanan) {
        $sql = "select id_grouprekanan from ms_rekanan where type_rekanan = '$typerekanan' and status = 1 order by CAST(coalesce(id_grouprekanan, '0') AS integer) DESC limit 1";
    ...
    function get_acl($sdm, $status, $nama_akses) {
        // status 1 = Menu, 2 = Sub Menu / Tab , 3 = Button / Function
        $cek_akses = $this->get_query("select count(*) as cek from akses_sdm_group 
        ...
        where asg_sdm_id = '$sdm' and status = $status and id_akses = '$nama_akses';", 1)->cek;
        return $cek_akses;
    }
    ...
    function hitungumur_lengkap($tgl_lahir, $tglperiksa, $status = null) {
        ...
        // YM = Bulan Tahun
        if ($status == null) {
            if ($ubulan == '0' && $utahun == '0') {
                $usia = $uhari . ' Hari';
            } else if ($utahun == '0' && $ubulan != '0') {
                $usia = $ubulan . ' Bln ' . $uhari . ' Hari';
            } else {
                $usia = $utahun . ' Thn ' . $ubulan . ' Bln ';
            }
        } else if ($status == 'YM') {
            $usia = $utahun . ' Thn ' . $ubulan . ' Bln ';
        } else if ($status == 'Y') {
    ...
    function cari_nilai_normal($kodepx, $umur, $data) {
        ...
        $where['kode_pemeriksaan'] = $data['kode_pemeriksaan'];
...
//        $this->get_log();
        foreach ($data_px as $row) {
            $opr_akhir = str_replace(' ', '', $row->usia_opr_akhir);
            $opr_awal = str_replace(' ', '', $row->usia_opr_awal);
...
//            log_message('error', $usia);
            // Cek
            if ($opr_awal == '' && $opr_akhir == '') {
//                log_message('error', 'Siji');
                $hasil = 't';
            } else if ($opr_akhir == '') {
//                log_message('error', 'loro');
                $hasil = $this->get_query("select $usia $opr_awal $awal  as total", 1)->total;
//                $this->get_log();
            } else if ($opr_awal == '') {
//                log_message('error', 'telu');
                $hasil = $this->get_query("select $usia $opr_akhir $akhir  as total", 1)->total;
//                $this->get_log();
            } else {
//                log_message('error', 'Papat');
                $hasil = $this->get_query("select $usia $opr_awal $awal and $usia $opr_akhir $akhir  as total", 1)->total;
//                $this->get_log();
            }
            log_message('error', $hasil);
            // Hasil
            if ($hasil == 't') {
                $rowcek = $this->get_query("select status_postingnilairujukan from trx_hasil where no_lab = '" . $data['no_lab'] . "' and kode_hasil = '$kodepx'", 1)->status_postingnilairujukan;
...
//                $cek_status = $rowcek->status_postingnilairujukan;
    ...
    function cari_nilai_normalx($kodepx, $umur, $data) {
        $insert = array();
        $where['no_lab'] = $data['no_lab'];
        $where['kode_hasil'] = $kodepx;
        $update['th_tglsample'] = $data['tgl_sample'];
...
//            $this->get_log_det($pecah2);
    ...
    public function cek_nilai_normal_new($hasil, $opr_awal, $val_awal, $opr_akhir, $val_akhir) {
        if ($opr_akhir == '') {
            $hasil = $this->get_query("select $hasil $opr_awal $val_awal  as total", 1)->total;
        } else if ($opr_awal == '') {
            $hasil = $this->get_query("select $hasil $opr_akhir $val_akhir  as total", 1)->total;
        } else {
            $hasil = $this->get_query("select $hasil $opr_awal $val_awal and $hasil $opr_akhir $val_akhir  as total");
        }
        return $hasil;
    }

    public function cek_nilai_normal($nolab, $kode_hasil, $th_hasil) {
        if (count(explode('.', $th_hasil)) > 1) {
            $conv_hasil = str_replace('.', '', $th_hasil);
        } else if (count(explode(',', $th_hasil)) > 1) {
            $conv_hasil = str_replace(',', '.', $th_hasil);
        } else {
            $conv_hasil = $th_hasil;
        }
...
//                                // Convert Value Awal
//                                if (count(explode(',', $nilairujukan->th_kuantitatif_val_awal)) > 0) {
//                                    $conv_valawal = str_replace(',', '.', $nilairujukan->th_kuantitatif_val_awal);
//                                } else if (count(explode('.', $nilairujukan->th_kuantitatif_val_awal)) > 0) {
//                                    $conv_valawal = str_replace('.', '', $nilairujukan->th_kuantitatif_val_awal);
//                                } else {
//                                    $conv_valawal = $nilairujukan->th_kuantitatif_val_awal;
//                                }
//                                // Convert Value Akhir
//                                 if (count(explode(',', $nilairujukan->th_kuantitatif_val_akhir)) > 0) {
//                                    $conv_valakhir = str_replace(',', '.', $nilairujukan->th_kuantitatif_val_akhir);
//                                } else if (count(explode('.', $nilairujukan->th_kuantitatif_val_akhir)) > 0) {
//                                    $conv_valakhir = str_replace('.', '', $nilairujukan->th_kuantitatif_val_akhir);
//                                } else {
//                                    $conv_valakhir = $nilairujukan->th_kuantitatif_val_akhir;
//                                }
//                                //
                $hasil_nilairujukan = $conv_hasil;
//                        $this->get_log_det($hasil_nilairujukan);
//                                $nilairujukan_valawal = $conv_valawal;
//                                $nilairujukan_valakhir = $conv_valakhir;
                // Tanpa Convert
//                                    $hasil_nilairujukan = $row->th_hasil;
...
//        log_message('error', $status_updown);
    ...
    public function get_reward($nolab, $group, $jenis, $format = null, $bintang = null, $reward_pergroup = null) {
        // Format
        // 0 Format Rupiah
        // 1 Normal

        $data_harga = $this->cabang->get_query_grid("select * from trx_pemeriksaan where no_lab = '$nolab' and jenispemeriksaan = '$group' and status_aktif = 1");
//$this->cabang->get_log();
        $status = 0;
        $berreward = 0;
        $reward = 0;
        $total = 0;
        foreach ($data_harga as $value) {
            // Cek Discount Lebih Besar
            if ($value->tp_discount >= $value->max_diskon) {
                $status += 1;
            } else {
                $status += 0;
            }
            // Cek Kurang Bayar
            $kb = $this->cabang->get_query("select rp_kurangbayar from trx_pasien where nolab = '$nolab'", 1)->rp_kurangbayar;
            if ($kb > 0) {
                $status += 1;
            } else {
                $status += 0;
            }
            // Cek Sudah Di Kerjakan atau Belum
            $ctabung = $this->cabang->get_query("select count(status) as count from ms_lab_pemeriksaan_tabung "
                            . "left join trx_sample on trx_sample.id_tabung = ms_lab_pemeriksaan_tabung.id_tabung "
                            . "where ms_lab_pemeriksaan_tabung.kode_pemeriksaan = '$value->kode_pemeriksaan' "
                            . "and no_lab = '$nolab'", 1)->count;
            $this->cabang->get_log();
            if ($ctabung > 0) {
                $tabung = $this->cabang->get_query("select status from ms_lab_pemeriksaan_tabung "
                        . "left join trx_sample on trx_sample.id_tabung = ms_lab_pemeriksaan_tabung.id_tabung "
                        . "where ms_lab_pemeriksaan_tabung.kode_pemeriksaan = '$value->kode_pemeriksaan' "
                        . "and no_lab = '$nolab' order by status DESC limit 1", 1);
                $this->cabang->get_log();
                if ($tabung->status < 1) {
                    $status += 1;
                } else {
            ...
            // Hitung Yg Berreward dan Reward
            if ($status == 0) {
                $berreward += $value->tp_harganett;
                // Reward
                $reward += ($value->tp_harganett * ($value->ps_dokter / 100));
            } else {
                $berreward += 0;
                $reward += 0;
            }
        ...
        if ($jenis == 1) {
            // Untuk Cek Omzet
            $query = $this->cabang->changenull($this->cabang->get_query("select sum(tp_harganett) as reward from trx_pemeriksaan where no_lab = '$nolab' and jenispemeriksaan = '$group' and status_aktif = 1", 1)->reward);
            if ($format == null) {
                if ($status > 0) {
                    $rp = number_format($query, 0, ',', '.') . "*";
                } else {
                    $rp = number_format($query, 0, ',', '.');
                }
            } else {
                // Cek Status Yang Berreward
                if ($bintang == null) {
                    $rp = $query;
                } else {
                    // Cek Per PX
                    if ($reward_pergroup == null) {
                        if ($status > 0) {
                            $rp = 1;
                        } else {
                            $rp = 0;
                        ...
                        if ($status > 0) {
                            ...
                            $rp = $this->cabang->changenull($this->cabang->get_query("select sum(tp_harganett * (ps_dokter / 100)) as reward from trx_pemeriksaan where no_lab = '$nolab' and jenispemeriksaan = '$group' and status_aktif = 1", 1)->reward);
                        }
                    }
                }
            }
            ...
            // Reward
            $rp = $this->cabang->changenull($reward);
        } else if ($jenis == 3) {
            // Yang Berreward
            $rp = $this->cabang->changenull($berreward);
//            $rp = $this->cabang->changenull($this->cabang->get_query("select sum(tp_harganett) as reward from trx_pemeriksaan where no_lab = '$nolab' and jenispemeriksaan = '$group' and status_aktif = 1 and tp_discount < max_diskon", 1)->reward);
        } else if ($jenis == 4) {
            // Total
            $harga = $this->cabang->get_query_grid("select * from trx_pemeriksaan where no_lab = '$nolab' and status_aktif = 1");
            foreach ($harga as $value1) {
                $total += $value1->tp_harganett;
            }
            $rp = number_format($total, 0, ',', '.');
        }
        return $rp;
    }
	public function insert_krebo($data) {
        $mydb = $this->db2;
