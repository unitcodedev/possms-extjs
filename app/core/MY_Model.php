...
class MY_Model extends CI_Model {
    ...
    public function generate_user_log($user, $cabang, $action, $form_id) {
        $user_detail = $this->get_detail('id', $user, 'ms_karyawan');
        if ($action == 'LOGIN' || $action == 'LOGOUT') {
            $generate = strtoupper($user_detail->mk_nama) . ' has been ' . $action;
        } else {
            $generate = strtoupper($user_detail->mk_nama) . ' ' . $action . ' data in ' . $form_id;
        }
        $data = array(
