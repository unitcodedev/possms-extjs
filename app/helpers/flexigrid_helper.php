...
if (! function_exists('build_grid_js'))
{
	 ...
	 *	 		* 1 - bclass
	 *	 		* 2 - onpress
         * @param string $onsubmit variabel for onsubmit javascript function
         * @param array $action variable for action javascript function
	 * @param	default sort column name
	 * @param	default sort order
	 * @param	array with aditional parameters
	 * @return	string
	 */
	function build_grid_js($grid_id,$url,$colModel,$sortname,$sortorder,$gridParams = NULL,$buttons = NULL,$onsubmit = NULL, $action = NULL)
	{
		//Basic propreties
		...
		if (!is_null($onsubmit)) $grid_js .= "onSubmit: ".$onsubmit.",";
		...
		//Penambahan apabila tidak ada searching
		...
		$grid_js .= $searchitems;
                ...
                //Add script for other javascript function
                
                if(is_array($action)){
                    foreach ($action as $function => $script){
                        //echo "$function $script";
                        $grid_js .= "$function : $script,";
                    }
                }
